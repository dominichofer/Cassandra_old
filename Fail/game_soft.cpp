...
int ZWS  (const uint64_t P, const uint64_t O, uint64_t& NodeCounter, const int alpha, const int selectivity, const int depth, const int empties)
{
	...
	for (const auto& mv : mvList) // ETC ESC
	{
		int score;
	...
	{
		score = MAX(score, -ZWS(mv.P, mv.O, NodeCounter, -alpha-1, selectivity, depth-1, empties-1));
		if (score > alpha)
		{
            if (mv.move == mvList.BestMove())
...
int PVS  (const uint64_t P, const uint64_t O, uint64_t& NodeCounter, int alpha, const int beta, const int selectivity, const int depth, const int empties, CLine* pline)
{
	...
	if (StabilityCutoff_PVS(P, O, alpha, beta, score)) return score;
	...
	for (const auto& mv : mvList)
	{
		if (SearchPV)
		{
			score = MAX(score, -PVS(mv.P, mv.O, NodeCounter, -beta, -alpha, selectivity, depth-1, empties-1, line));
...
namespace Midgame
{
	...
	int ZWS_1(const uint64_t P, const uint64_t O, uint64_t& NodeCounter, const int alpha)
	{
		...
		while (BitBoardPossible)
		{
			...
			score = MAX(score, -ZWS_0(O ^ flipped, P ^ (1ULL << Move) ^ flipped, NodeCounter));
	...
	int ZWS_2(const uint64_t P, const uint64_t O, uint64_t& NodeCounter, const int alpha)
	{
		...
		while (BitBoardPossible)
		{
			...
			RemoveLSB(BitBoardPossible);
			flipped = flip(P, O, Move);
			score = MAX(score, -ZWS_1(O ^ flipped, P ^ (1ULL << Move) ^ flipped, NodeCounter, -alpha-1));
			if (score > alpha) return score;
		}
		return score;
	}
	int ZWS_3(const uint64_t P, const uint64_t O, uint64_t& NodeCounter, const int alpha)
	{
		...
		while (BBTmp)
		{
			...
			RemoveLSB(BBTmp);
			flipped = flip(P, O, Move);
			score = MAX(score, -ZWS_2(O ^ flipped, P ^ (1ULL << Move) ^ flipped, NodeCounter, -alpha - 1));
			if (score > alpha) return score;
		}
		BBTmp = BitBoardPossible &  BBParity & ~PATTERN_FIRST;
		while (BBTmp)
	...
	int PVS_1(const uint64_t P, const uint64_t O, uint64_t& NodeCounter, int alpha, const int beta, CLine* pline)
	{
		...
		while (BitBoardPossible)
		{
			...
			if (SearchPV)
			{
				score = MAX(score, -PVS_0(O ^ flipped, P ^ (1ULL << Move) ^ flipped, NodeCounter));
			...
			if (score >= beta) { delete line; return score; }
			if (score > alpha) { alpha = score; SearchPV = false; }
		}
	...
	int PVS_2(const uint64_t P, const uint64_t O, uint64_t& NodeCounter, int alpha, const int beta, CLine* pline)
	{
		...
		for (const auto& mv : mvList)
		{
			...
			if (SearchPV)
			{
				score = MAX(score, -PVS_1(O ^ flipped, P ^ (1ULL << mv) ^ flipped, NodeCounter, -beta, -alpha, line));
				if (line) pline->NewPV(mv, line);
			}
			else
			{
				score = MAX(score, -ZWS_1(O ^ flipped, P ^ (1ULL << mv) ^ flipped, NodeCounter, -alpha-1));
				if (score > alpha)
				{
					score = MAX(score, -PVS_1(O ^ flipped, P ^ (1ULL << mv) ^ flipped, NodeCounter, -beta, -alpha, line));
					if (line) pline->NewPV(mv, line);
	...
	int PVS_3(const uint64_t P, const uint64_t O, uint64_t& NodeCounter, int alpha, const int beta, CLine* pline)
	{
		...
		for (const auto& mv : mvList)
		{
			...
			if (SearchPV)
			{
				score = MAX(score, -PVS_2(O ^ flipped, P ^ (1ULL << mv) ^ flipped, NodeCounter, -beta, -alpha, line));
				if (line) pline->NewPV(mv, line);
			}
			else
			{
				score = MAX(score, -ZWS_2(O ^ flipped, P ^ (1ULL << mv) ^ flipped, NodeCounter, -alpha-1));
				if (score > alpha)
				{
					score = MAX(score, -PVS_2(O ^ flipped, P ^ (1ULL << mv) ^ flipped, NodeCounter, -beta, -alpha, line));
					if (line) pline->NewPV(mv, line);
...
namespace Endgame
{
	int ZWS_1(const uint64_t P, const uint64_t O, uint64_t& NodeCounter, const int alpha, const unsigned int x)
	{
		...
		else
		{
			if (score < alpha)
				return score;
			else if (diff = count_last_flip(O, x))
			{
	...
	int ZWS_2(const uint64_t P, const uint64_t O, uint64_t& NodeCounter, const int alpha, const unsigned int x1, const unsigned int x2)
	{
		...
		{
			score = MAX(score, -ZWS_1(O ^ flipped, P ^ (1ULL << x1) ^ flipped, NodeCounter, -alpha - 1, x2));
			if (score > alpha) return score;
			played = true;
		}
		...
		if ((O & neighbour[x2]) && (flipped = flip(P, O, x2)))
			return MAX(score, -ZWS_1(O ^ flipped, P ^ (1ULL << x2) ^ flipped, NodeCounter, -alpha-1, x1));
		...
		{
			score = MIN(score, ZWS_1(P ^ flipped, O ^ (1ULL << x1) ^ flipped, NodeCounter, alpha, x2));
			if (score <= alpha) return score;
			played = true;
		}
		...
		if ((P & neighbour[x2]) && (flipped = flip(O, P, x2)))
			return MIN(score, ZWS_1(P ^ flipped, O ^ (1ULL << x2) ^ flipped, NodeCounter, alpha, x1));
	...
	int ZWS_3(const uint64_t P, const uint64_t O, uint64_t& NodeCounter, const int alpha, const unsigned int x1, const unsigned int x2, const unsigned int x3)
	{
		...
		if ((O & neighbour[x1]) && (flipped = flip(P, O, x1)))
		{
			score = MAX(score, -ZWS_2(O ^ flipped, P ^ (1ULL << x1) ^ flipped, NodeCounter, -alpha - 1, x2, x3));
			if (score > alpha) return score;
			played = true;
		}
		...
		if ((O & neighbour[x2]) && (flipped = flip(P, O, x2)))
		{
			score = MAX(score, -ZWS_2(O ^ flipped, P ^ (1ULL << x2) ^ flipped, NodeCounter, -alpha - 1, x1, x3));
			if (score > alpha) return score;
			played = true;
		}
		...
		if ((O & neighbour[x3]) && (flipped = flip(P, O, x3)))
			return MAX(score, -ZWS_2(O ^ flipped, P ^ (1ULL << x3) ^ flipped, NodeCounter, -alpha - 1, x1, x2));
		...
		if ((P & neighbour[x1]) && (flipped = flip(O, P, x1)))
		{
			score = MIN(score, ZWS_2(P ^ flipped, O ^ (1ULL << x1) ^ flipped, NodeCounter, alpha, x2, x3));
			if (score <= alpha) return score;
			played = true;
		}
		...
		if ((P & neighbour[x2]) && (flipped = flip(O, P, x2)))
		{
			score = MIN(score, ZWS_2(P ^ flipped, O ^ (1ULL << x2) ^ flipped, NodeCounter, alpha, x1, x3));
			if (score <= alpha) return score;
			played = true;
		}
		...
		if ((P & neighbour[x3]) && (flipped = flip(O, P, x3)))
			return MIN(score, ZWS_2(P ^ flipped, O ^ (1ULL << x3) ^ flipped, NodeCounter, alpha, x1, x2));
	...
	int ZWS_4(const uint64_t P, const uint64_t O, uint64_t& NodeCounter, const int alpha, const unsigned int x1, const unsigned int x2, const unsigned int x3, const unsigned int x4)
	{
		...
		if ((O & neighbour[x1]) && (flipped = flip(P, O, x1)))
		{
			score = MAX(score, -ZWS_3(O ^ flipped, P ^ (1ULL << x1) ^ flipped, NodeCounter, -alpha - 1, x2, x3, x4));
			if (score > alpha) return score;
			played = true;
		}
		...
		if ((O & neighbour[x2]) && (flipped = flip(P, O, x2)))
		{
			score = MAX(score, -ZWS_3(O ^ flipped, P ^ (1ULL << x2) ^ flipped, NodeCounter, -alpha - 1, x1, x3, x4));
			if (score > alpha) return score;
			played = true;
		}
		...
		if ((O & neighbour[x3]) && (flipped = flip(P, O, x3)))
		{
			score = MAX(score, -ZWS_3(O ^ flipped, P ^ (1ULL << x3) ^ flipped, NodeCounter, -alpha - 1, x1, x2, x4));
			if (score > alpha) return score;
			played = true;
		}
		...
		if ((O & neighbour[x4]) && (flipped = flip(P, O, x4)))
			return MAX(score, -ZWS_3(O ^ flipped, P ^ (1ULL << x4) ^ flipped, NodeCounter, -alpha - 1, x1, x2, x3));
		...
		if ((P & neighbour[x1]) && (flipped = flip(O, P, x1)))
		{
			score = MIN(score, ZWS_3(P ^ flipped, O ^ (1ULL << x1) ^ flipped, NodeCounter, alpha, x2, x3, x4));
			if (score <= alpha) return score;
			played = true;
		}
		...
		if ((P & neighbour[x2]) && (flipped = flip(O, P, x2)))
		{
			score = MIN(score, ZWS_3(P ^ flipped, O ^ (1ULL << x2) ^ flipped, NodeCounter, alpha, x1, x3, x4));
			if (score <= alpha) return score;
			played = true;
		}
		...
		if ((P & neighbour[x3]) && (flipped = flip(O, P, x3)))
		{
			score = MIN(score, ZWS_3(P ^ flipped, O ^ (1ULL << x3) ^ flipped, NodeCounter, alpha, x1, x2, x4));
			if (score <= alpha) return score;
			played = true;
		}
		...
		if ((P & neighbour[x4]) && (flipped = flip(O, P, x4)))
			return MIN(score, ZWS_3(P ^ flipped, O ^ (1ULL << x4) ^ flipped, NodeCounter, alpha, x1, x2, x3));

		if (played) return alpha+1;
	...
	int ZWS_A(const uint64_t P, const uint64_t O, uint64_t& NodeCounter, const int alpha, const int empties)
	{
		...
		while (BBTmp)
		{
			...
			RemoveLSB(BBTmp);
			flipped = flip(P, O, Move);
			score = MAX(score, -ZWS_A(O ^ flipped, P ^ (1ULL << Move) ^ flipped, NodeCounter, -alpha-1, empties-1));
			if (score > alpha) return score;
		}
	...
	int PVS_A(const uint64_t P, const uint64_t O, uint64_t& NodeCounter,       int alpha, const int beta, const int empties, CLine* pline)
	{
		...
		for (const auto& mv : mvList)
		{
			...
			if (SearchPV)
			{
				score = MAX(score, -PVS_A(O ^ flipped, P ^ (1ULL << mv) ^ flipped, NodeCounter, -beta, -alpha, empties-1, line));
				if (line) pline->NewPV(mv, line);
			}
			else
			{
				     if (empties == 2) score = MAX(score, -ZWS_1(O ^ flipped, P ^ (1ULL << mv) ^ flipped, NodeCounter, -alpha-1));
				else if (empties == 3) score = MAX(score, -ZWS_2(O ^ flipped, P ^ (1ULL << mv) ^ flipped, NodeCounter, -alpha-1));
				else if (empties == 4) score = MAX(score, -ZWS_3(O ^ flipped, P ^ (1ULL << mv) ^ flipped, NodeCounter, -alpha-1));
				else if (empties == 5) score = MAX(score, -ZWS_4(O ^ flipped, P ^ (1ULL << mv) ^ flipped, NodeCounter, -alpha-1));
				else                   score = MAX(score, -ZWS_A(O ^ flipped, P ^ (1ULL << mv) ^ flipped, NodeCounter, -alpha-1, empties-1));
